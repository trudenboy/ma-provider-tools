...
# Marker injected at the top of rendered CLAUDE.md so a future distribute run
# can tell apart a template-rendered file from one a maintainer wrote by hand.
...
def create_pr_for_provider(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # One-shot CLAUDE.md migration: when the provider already has a hand-authored
        # CLAUDE.md and no CLAUDE.local.md, move the maintainer's content to
        # CLAUDE.local.md before the template overwrites CLAUDE.md.
        changed = False
        if "CLAUDE.md" in rendered:
            existing_claude = Path(tmpdir) / "CLAUDE.md"
            local_claude = Path(tmpdir) / "CLAUDE.local.md"
            if existing_claude.exists() and not local_claude.exists():
                content = existing_claude.read_text()
                if CLAUDE_RENDERED_MARKER not in content:
                    local_claude.write_text(content)
